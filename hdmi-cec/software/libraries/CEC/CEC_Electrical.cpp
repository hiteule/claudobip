...
/**
 * Implements our main state machine which includes all reading and writing of state including acknowledgements and arbitration
 */
unsigned long CEC_Electrical::Process()
{
  // We are either called because of an INTerrupt in which case state has changed or because of a poll (during write).
  ...
  if( currentLineState == lastLineState &&
    !(
      ...
      (
        ...
        (_secondaryState == CEC_RCV_ACK_SENT || _secondaryState == CEC_RCV_LINEERROR)
      )
    )
  ) {
    return waitTime;
  ...
  switch (_primaryState) {
    case CEC_IDLE:
      // If a high to low transition occurs, then we must be beginning a start bit
      ...
      // Nothing to do until we have a need to transmit or we detect the falling edge of the start bit
      break;
    ...
    case CEC_RECEIVE:
      switch (_secondaryState) {
        ...
        case CEC_RCV_DATABIT2:
          ...
          if (difftime >= 2050 && difftime <= 2750) {
            if (_tertiaryState == CEC_RCV_BIT_EOM) {
              ...
              // Check to see if the frame is addressed to us or if we are in promiscuous mode (in which case we'll receive everything)
        ...
        case CEC_RCV_ACK_SENT:
          ...
          if (_eom) {
            ...
            // And we've sent the ACK for the most recent bit therefore this message is all done.
        ...
        // Code currently exists in CEC_RCV_DATABIT2 that checks the address of the frame and if it isn't addressed to this device it goes back to watching for a start bit state. 
        // This state, CEC_RCV_ACK1, was from when we didn't do that and we followed state for all frames regardless of addressing.
        // However, I'm not removing this code because it will be needed when we support broadcast frames.
        case CEC_RCV_ACK1:
          {
            int ack;
            if (difftime >= 400 && difftime <= 800) {
            ...
            if (_eom && ack == CEC_ACK) {
              // We're not going to receive anything more from the initiator (EOM has been received)
              // And we've seen the ACK for the most recent bit therefore this message is all done.
              // Go back to the IDLE state and wait for another start bit.
              ProcessFrame();
              waitTime = ResetState() ? micros() : (unsigned long)-1;
              break;
      ...
      break; // End CEC_RECEIVE

    case CEC_TRANSMIT:
      if (lastLineState != currentLineState) {
        // Someone else is mucking with the line. Wait for the line to clear before appropriately before (re)transmit
        ...
        if (_secondaryState != CEC_XMIT_ACK &&
          ...
          // This is a state change from an ACK and isn't part of our state tracking.
          waitTime = -2;
          break;
        }
      }
      ...
      switch (_secondaryState) {
        case CEC_IDLE_WAIT:
          ...
          // If the line is low, we can't do anything now.
          // Wait indefinitely until a line state changes which will catch in the code just above.
        ...
        case CEC_XMIT_DATABIT1:
          ...
          if (_tertiaryState == CEC_XMIT_BIT_EOM) {
            // We've just finished transmitting the EOM move on to the ACK
        ...
        case CEC_XMIT_ACK_TEST:
          if ((currentLineState != 0) != _broadcast) {
            ...
            // Normally we retransmit.  But this is NOT the case for <Polling Message> as its function is basically to 'ping' a logical address
            // in which case we just want acknowledgement that it has succeeded or failed.
